...
| Public repository | Public release: https://github.com/febert0001-tech/changegate-webmcp. GitHub reports the repository as public and detects its MIT license. |
| Open-source license | MIT `LICENSE` is present. |
| Demo video | Public 2:32 YouTube demo with narration and licensed background music: https://youtu.be/LVBfupdF-5U |

Official extended submission deadline: **September 4, 2026 at 1:00 a.m. Pacific Time (4:00 a.m. Eastern Time)**. Devpost announced a 12-hour extension because of an outage.
...
- Final production judge-path smoke test passed: seven native tools discovered; `$25.00` refund proposed for Order #4821; human approval remained non-executing; separate human Execute completed; independent ledger readback reached `SUCCEEDED` / **VERIFIED**.
...
- Background music (`Steady Grid`) was generated by the entrant through an active Suno Pro subscription, which grants commercial-use rights for outputs created during the paid subscription term.
- The demo narration uses a licensed neutral synthetic voice rather than a clone or imitation of the entrant or another person.
- The video uses project-owned product visuals and only the minimum platform/browser interface needed to demonstrate native WebMCP operation.

## Final release status

1. Public documentation reviewed.
2. GitHub repository confirmed public; MIT license detected.
3. Required public YouTube demo published at 2:32 with audio.
4. Required Devpost fields completed and the entry submitted.
5. Public live URL and native WebMCP success path re-tested successfully.

After the extended submission period closes, the submitted Devpost entry, repository, video, and live site must remain frozen through judging. Future development should use a separate fork/copy.
